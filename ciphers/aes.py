...
def key_expansion(key):
    ...
    for i in range(4, 4 * 11):
        ...
        if i % 4 == 0:
            ...
            temp[0] ^= Rcon[i // 4]
        for j in range(4):
            w.append(w[4 * (i - 4) + j] ^ temp[j])
            
    round_keys = []
    for i in range(11):
        round_keys.append([w[16 * i + j: 16 * i + j + 4] for j in range(0, 16, 4)])
    return round_keys

def encrypt_block(state, key_schedule):
...
def decrypt_block(state, key_schedule):
    add_round_key(state, key_schedule[10])
...
def encrypt_manual(text, key):
    ...
    for i in range(0, len(padded_text), 16):
        ...
        state = [[ord(block[r + 4 * c]) for r in range(4)] for c in range(4)]
        state = encrypt_block(state, key_schedule)
        for c in range(4):
            for r in range(4):
...
def decrypt_manual(text, key):
    ...
    for i in range(0, len(data), 16):
        ...
        state = decrypt_block(state, key_schedule)
        for c in range(4):
            for r in range(4):
                decrypted_bytes.append(state[c][r])
                
    decrypted_str = "".join(chr(x) for x in decrypted_bytes)
